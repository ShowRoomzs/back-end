...
--
-- PENDING_KEY는 CASE WHEN status='PENDING' THEN 1 END 로 만든 생성 컬럼이다.
...
-- IF() 대신 CASE를 쓴다 — Flyway MySQL 파서가 IF( 를 조건 구문으로 오인해 CI(빈 DB)에서
-- CREATE TABLE이 깨지는 것을 막기 위함이다. V90의 COALESCE 생성 컬럼과 같은 목적이다.
CREATE TABLE `brand_change_request` (
    ...
    `pending_key`             BIGINT GENERATED ALWAYS AS (CASE WHEN `status` = 'PENDING' THEN 1 ELSE NULL END) STORED,
